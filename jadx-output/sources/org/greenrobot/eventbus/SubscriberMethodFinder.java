        ...
        for (Method method : methods) {
            ...
            if ((modifiers & 1) != 0 && (modifiers & MODIFIERS_IGNORE) == 0) {
                ...
                if (parameterTypes.length == 1) {
                    ...
                    StringBuilder sbC = jo.C("@Subscribe method ", method.getDeclaringClass().getName() + "." + method.getName(), "must have exactly 1 parameter but has ");
                    sbC.append(parameterTypes.length);
                    throw new EventBusException(sbC.toString());
                }
            } else if (this.strictMethodVerification && method.isAnnotationPresent(Subscribe.class)) {
                throw new EventBusException(jo.n(method.getDeclaringClass().getName() + "." + method.getName(), " is a illegal @Subscribe method: must be public, non-static, and non-abstract"));
            }
        }
    }
