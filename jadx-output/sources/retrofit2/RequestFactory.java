...
public final class RequestFactory {
    ...
    public static final class Builder {
        ...
        private Headers parseHeaders(String[] strArr) {
            ...
            for (String str : strArr) {
                ...
                if (HttpConnection.CONTENT_TYPE.equalsIgnoreCase(strSubstring)) {
                    try {
        ...
        private ParameterHandler<?> parseParameter(int i, Type type, Annotation[] annotationArr, boolean z) {
            ...
            if (z) {
                try {
                    if (Utils.getRawType(type) == pk7.class) {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i, Type type, Annotation[] annotationArr, Annotation annotation) {
            ...
            if (annotation instanceof Part) {
                ...
                Headers headersOf = Headers.of("Content-Disposition", jo.o("form-data; name=\"", strValue5, "\""), "Content-Transfer-Encoding", part.encoding());
                if (!Iterable.class.isAssignableFrom(rawType8)) {
                    if (!rawType8.isArray()) {
                        if (MultipartBody.Part.class.isAssignableFrom(rawType8)) {
            ...
            for (int i2 = i - 1; i2 >= 0; i2--) {
                ...
                if ((parameterHandler instanceof ParameterHandler.Tag) && ((ParameterHandler.Tag) parameterHandler).cls.equals(rawType10)) {
                    ...
                    StringBuilder sbZ = jo.z("@Tag type ");
                    sbZ.append(rawType10.getName());
                    sbZ.append(" is duplicate of parameter #");
                    sbZ.append(i2 + 1);
                    sbZ.append(" and would always overwrite its value.");
                    throw Utils.parameterError(method, i, sbZ.toString(), new Object[0]);
                }
            }
    ...
    public Request create(Object[] objArr) throws IOException {
        ...
        if (length != parameterHandlerArr.length) {
            throw new IllegalArgumentException(jo.q(jo.A("Argument count (", length, ") doesn't match expected count ("), parameterHandlerArr.length, ")"));
        }
